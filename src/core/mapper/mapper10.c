...
static void mapper10_init_ops(NesMapper *mapper, NesMirroring mirroring)
{
    (void)mirroring;
    nes_mapper10_init(mapper);
}

static int mapper10_cpu_read(NesEmu *nes, uint16_t address, uint8_t *value)
{
    ...
    if (address >= 0x8000u) {
        *value = nes_mapper10_prg_read(&nes->mapper, address);
        return 1;
    }
    return 0;
}

static int mapper10_cpu_write(NesEmu *nes, uint16_t address, uint8_t value)
{
    if (nes == NULL) {
        return 0;
    }
...
static int mapper10_ppu_read(NesEmu *nes, uint16_t address, uint8_t *value)
{
    if (nes == NULL || value == NULL || address >= 0x2000u) {
        return 0;
    }
    *value = nes_mapper10_ppu_read(nes, address);
    return 1;
}

static int mapper10_ppu_write(NesEmu *nes, uint16_t address, uint8_t value)
{
...
const NesMapperOps nes_mapper10_ops = {
    10,
    mapper10_init_ops,
    mapper10_cpu_read,
    mapper10_cpu_write,
